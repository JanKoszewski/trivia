...
describe "UglyGame::Trivia" do
  let(:game)  { UglyTrivia::Game.new }

  describe "#run" do
    context "under standardized constraints" do
      it "should return deterministic output" do
        srand(100000)
        ...
        expect(game.run).to eq('Sue has won the game!')
      end
    end
  end
  ...
  describe "full output" do
    context "under standardized constraints" do
      it "should retrun deterministic output" do
        output = `bin/trivia`
        expect(output).to eq(
          <<~HEREDOC
            Chet was added
